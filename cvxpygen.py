...
def generate_code(problem, code_dir='CPG_code', compile_module=True, explicit=False, problem_name=''):
    ...
    # get problem data
    data, solving_chain, inverse_data = problem.get_problem_data(solver='OSQP', gp=False, enforce_dpp=True,
                                                                 verbose=False)
    n_var = data['n_var']
    n_eq = data['n_eq']
    n_ineq = data['n_ineq']
    ...
    with open(os.path.join(code_dir, 'c', 'include', 'cpg_workspace.h'), 'a') as f:
        utils.write_workspace_prot(f, explicit, user_p_names, user_p_writable, user_p_flat, var_init, osqp_p_ids,
                                   osqp_p, osqp_mappings, var_symmetric)

    # 'workspace' definitions
    with open(os.path.join(code_dir, 'c', 'src', 'cpg_workspace.c'), 'a') as f:
        utils.write_workspace_def(f, explicit, user_p_names, user_p_writable, user_p_flat, var_init, osqp_p_ids, osqp_p,
                                  osqp_mappings, var_symmetric, var_offsets)

    # 'solve' prototypes
    with open(os.path.join(code_dir, 'c', 'include', 'cpg_solve.h'), 'a') as f:
        utils.write_solve_prot(f, user_p_name_to_size, osqp_settings_names_to_types)

    # 'solve' definitions
    with open(os.path.join(code_dir, 'c', 'src', 'cpg_solve.c'), 'a') as f:
        utils.write_solve_def(f, explicit, osqp_p_ids, osqp_mappings, user_p_col_to_name,
                              ...
                              osqp_settings_names_to_types, var_symmetric, osqp_p_to_changes)
    ...
    myOSQP.codegen(os.path.join(code_dir, 'c', 'OSQP_code'), project_type=cmake_generator, parameters='matrices',
                   force_rewrite=True)
    ...
    with open(os.path.join(code_dir, 'cpp', 'include', 'cpg_module.hpp'), 'a') as f:
        utils.write_module_prot(f, user_p_name_to_size, var_name_to_size, problem_name)
